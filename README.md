...
| `--pack-ms`      | Chọn OnlyOffice, mở rộng swap và Flameshot; không chạy Basic chuyển KDE, các script và item khác chọn tất cả |
| `--pack-bs`      | Chọn Flatpak, IME, Lotus, mở rộng swap, OnlyOffice, Chrome, Mattermost, Flameshot và Claude Desktop; không chạy Basic chuyển KDE |
| `--keep-snap`    | Giữ Snap; Basic De-bloat tự bỏ de-snap trên Ubuntu và các flavors (VD Kubuntu), còn Tuxedo vẫn được de-brand; dùng được cùng các pack |
| `--help`, `-h` | In trợ giúp                                                                               |
...
- Chạy `sudo ./setup-ubuntu-based.sh` để chọn Basics và extras qua menu động
- Dùng `sudo ./setup-ubuntu-based.sh --all`, `--pack-ms` hoặc `--pack-bs` để chạy không tương tác; thêm `--keep-snap` để Basic De-bloat tự giữ Snap trên máy non-Tuxedo
- Các scripts con cũng có thể chạy độc lập; Basics và extras đều có thể chọn/bỏ qua từ parent
...
| `2-debloat-ubuntu-based.sh` | Như child core, kèm `--keep-snap` để giữ Snap ở non-Tuxedo |
